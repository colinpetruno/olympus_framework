module Scheduling
  class RescheduleBookingJob < ApplicationJob
    queue_as :bookings
    ...
    def perform(booking_id)
      ...
      external_ids.map do |external_id|
        ...
        # bookings can be synced to many calendars, thus for a reschedule we
        # need to map each external id and then find the calendar event for that
        ExternalResources::Events::Patch.update_booking(
