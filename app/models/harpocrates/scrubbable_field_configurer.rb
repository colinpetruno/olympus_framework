module Harpocrates
  class ScrubbableFieldConfigurer
    attr_reader :field
    ...
    def field_value(instance_object)
      if ["Symbol", "String"].include?(field.class.name)
        field_type = instance_object.column_for_attribute(field).type

        value_for(field_type)
    ...
    def field_options
      if ["Symbol", "String"].include?(field_name.class.name)
        {}
      elsif ["Hash"].include?(field.class.name)
    ...
    def value_for(field_type)
      if [:string].include?(field_type.to_sym)
        ...
        0
      elsif [:boolean].include?(field_type.to_sym)
        [true, false].sample
      else
